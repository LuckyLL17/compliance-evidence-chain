...
func (s *Service) CapacityFindings(seed string) []CapacityFinding {
	...
	if seed == "" {
		seed = "evidence-capacity"
	}
	now := s.clock.Now()
	findings := []CapacityFinding{
		{
			Code:      "evidence-capacity-baseline",
			Subject:   seed,
			Severity:  "info",
			Message:   "capacity and workload boundaries",
			Score:     len(seed) % 17,
			CreatedAt: now,
			Metadata:  map[string]string{"module": "capacity", "domain": "compliance-evidence-chain"},
		},
		{
			Code:      "evidence-capacity-freshness",
		...
		{
			Code:      "evidence-capacity-ownership",
