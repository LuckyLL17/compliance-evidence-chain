...
func (value AccessRule) Validate() error {
...
func (value AccessRule) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *AccessRule) Prepare(now time.Time) {
...
func (value *AccessRule) Advance(next Status, now time.Time) error {
...
func (value AccessRule) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "access-rules",
		Name:      value.Name,
