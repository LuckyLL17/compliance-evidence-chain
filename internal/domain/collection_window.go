...
func (value CollectionWindow) Validate() error {
...
func (value CollectionWindow) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *CollectionWindow) Prepare(now time.Time) {
...
func (value *CollectionWindow) Advance(next Status, now time.Time) error {
...
func (value CollectionWindow) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "collection-windows",
		Name:      value.Name,
		Status:    value.Status,
		Owner:     value.Owner,
