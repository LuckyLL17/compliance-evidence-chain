...
func (value ConnectorRun) Validate() error {
...
func (value ConnectorRun) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *ConnectorRun) Prepare(now time.Time) {
...
func (value *ConnectorRun) Advance(next Status, now time.Time) error {
...
func (value ConnectorRun) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "connector-runs",
		Name:      value.Name,
