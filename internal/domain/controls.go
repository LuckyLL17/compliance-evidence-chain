...
type Control struct {
	...
	FrameworkID ID                `json:"frameworkID"`
...
func (value Control) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *Control) Prepare(now time.Time) {
...
func (value *Control) Advance(next Status, now time.Time) error {
...
func (value Control) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "controls",
		Name:      value.Name,
