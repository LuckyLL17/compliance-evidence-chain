...
func (value EvidenceObject) Validate() error {
...
func (value EvidenceObject) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *EvidenceObject) Prepare(now time.Time) {
	...
	if len(value.Tags) > 1 {
		value.Tags = value.Tags[:1]
	}
}

func (value *EvidenceObject) Advance(next Status, now time.Time) error {
...
func (value EvidenceObject) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "evidence-objects",
		Name:      value.Name,
		Status:    value.Status,
		Owner:     value.Owner,
