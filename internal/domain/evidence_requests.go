...
type EvidenceRequest struct {
	...
	ControlID  ID                `json:"controlID"`
	DueAt      time.Time         `json:"dueAt"`
}

func (value EvidenceRequest) Validate() error {
...
func (value EvidenceRequest) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *EvidenceRequest) Prepare(now time.Time) {
...
func (value *EvidenceRequest) Advance(next Status, now time.Time) error {
...
func (value EvidenceRequest) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "evidence-requests",
		Name:      value.Name,
		Status:    value.Status,
		Owner:     value.Owner,
