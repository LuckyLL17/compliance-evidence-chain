...
func (value ExceptionCase) Validate() error {
...
func (value ExceptionCase) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *ExceptionCase) Prepare(now time.Time) {
...
func (value *ExceptionCase) Advance(next Status, now time.Time) error {
...
func (value ExceptionCase) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "exception-cases",
		Name:      value.Name,
		Status:    value.Status,
		Owner:     value.Owner,
