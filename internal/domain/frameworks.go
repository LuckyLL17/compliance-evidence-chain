...
func (value Framework) Validate() error {
...
func (value Framework) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *Framework) Prepare(now time.Time) {
...
func (value *Framework) Advance(next Status, now time.Time) error {
	if err := Transition(value.Status, next); err != nil {
		return err
	}
	value.Status = next
	value.ID = ID(strings.TrimSpace(string(value.ID)))
	if value.Owner == "" {
		value.Owner = "system"
	}
	value.Version++
...
func (value Framework) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "frameworks",
		Name:      value.Name,
