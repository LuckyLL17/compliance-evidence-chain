...
func (r *Router) handleControls(w http.ResponseWriter, req *http.Request) {
	parts := pathParts(req, "/api/v1/controls")
	if len(parts) == 0 {
		if req.Method == http.MethodGet {
			limit, _ := strconv.Atoi(req.URL.Query().Get("limit"))
			writeJSON(w, http.StatusOK, r.service.ListControls(req.URL.Query().Get("q"), limit))
			return
		}
		if req.Method == http.MethodPost {
			var input domain.Control
			if err := decodeJSON(req, &input); err != nil {
				writeError(w, http.StatusBadRequest, err)
				return
			}
			value, err := r.service.CreateControl(input)
			if err != nil {
	...
	if len(parts) == 1 && req.Method == http.MethodGet {
		value, ok := r.service.GetControl(id)
		if !ok {
	...
	if len(parts) == 2 && parts[1] == "advance" && req.Method == http.MethodPost {
		...
		value, err := r.service.AdvanceControl(id, input.Status, input.Actor)
		if err != nil {
